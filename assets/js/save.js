document.addEventListener("DOMContentLoaded", function () {
    ...
    let isSortedByPopularityAsc = false; // Флаг для отслеживания состояния сортировки
    ...
    function filterData(category) {
      ...
      if (category === "all") {
        ...
        fetch(`https://672885dc270bd0b97555ee35.mockapi.io/repos?category=${category}`)
          .then((response) => response.json())
          .then((data) => {
            allData = data;
            filteredData = allData;
    ...
    searchInput.addEventListener("input", () => {
      ...
      fetch(`https://672885dc270bd0b97555ee35.mockapi.io/repos?name=${currentSearchTerm}`)
        .then((response) => response.json())
        .then((data) => {
          allData = data;
          filteredData = allData;
    ...
    sortButton.addEventListener("click", () => {
      spiner.style.display = "flex"; // Показываем лоадер
      spinerB.style.display = "flex"; 
      isSortedByPopularityAsc = !isSortedByPopularityAsc; // Инвертируем флаг сортировки
